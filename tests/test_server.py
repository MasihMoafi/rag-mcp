...
def test_rag_tool_is_advertised_as_read_only() -> None:
    assert server._TOOL["name"] == "query_knowledge_base"
    assert server._TOOL["annotations"] == {
...
def test_empty_query_is_rejected_without_touching_rag() -> None:
    assert json.loads(server.query_knowledge_base("   ")) == {
